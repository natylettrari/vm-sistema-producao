...
// Cristal — agrupa pelo tamanho
function extrairModeloCristal(title) {
  const t = title.toLowerCase();
  if (!t.includes('cristal')) return null;
  // Detecta tamanho
  if (t.includes('pp')) return 'Kit Cristal PP';
  if (t.includes(' g') || t.includes('-g') || t.match(/cristal\s*g\b/i)) return 'Kit Cristal G';
  if (t.includes(' m') || t.includes('-m') || t.match(/cristal\s*m\b/i)) return 'Kit Cristal M';
  if (t.includes(' p') || t.includes('-p') || t.match(/cristal\s*p\b/i)) return 'Kit Cristal P';
  return 'Kit Cristal';
}
